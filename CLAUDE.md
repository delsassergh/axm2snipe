...
- `cmd/` — Cobra subcommands (sync, download, setup, test, backfill-images, access-token, request)
...
- ABM `fields[orgDevices]` is a JSON:API sparse fieldset — it filters attributes to only the listed fields, not adds to them. More surprisingly, it also appears to gate whether **released devices show up in the list at all**: with no `fields[orgDevices]` param (axm2snipe's behavior before this was fixed), a device confirmed "Released" in the ABM portal was completely absent from `/v1/orgDevices` — not present with an empty `releasedFromOrgDateTime`, just missing entirely. `sync.orgDeviceFields` now explicitly requests every field the sync engine reads, plus `releasedFromOrgDateTime`, to opt into getting released devices back. There is no distinct "RELEASED" status value — a released device has `status: UNASSIGNED` *and* `releasedFromOrgDateTime` set, confirmed via Apple's own example response. `releaserId`/`releaserEntityType` appear in some Apple documentation but are rejected outright by the live API (`PARAMETER_ERROR.INVALID: '...' is not a valid field name`) — don't add them back without testing against a live request first. If a stale `devices.progress.json` exists from a run before this fix, resuming it continues using the old (fieldless) request URL for whatever pages are left — use `download --restart` to force a clean pull with the new fields.
